...
[(raw_string) (ansi_c_string) (heredoc_body)] @string
...
((command (command_name (word) @function.call)))
((test_command [ "[[" "]]" ] @function.call))
((arithmetic_expansion ["((" "))"] @function.call))
((command
   name: (command_name (word))
   argument: [(word) @variable.member
              (concatenation (word) @variable.member)])
...
[ "declare" "typeset" "readonly" "local" "unset" "unsetenv" ] @keyword
...
(command_substitution "$(" @punctuation.special ")" @punctuation.special)

(process_substitution [ "<(" ">(" ] @punctuation.special ")" @punctuation.special)

(arithmetic_expansion [ "$((" "((" ] @punctuation.special "))" @punctuation.special)
...
((unset_command (word) @variable.parameter))
...
(file_redirect (word) @string.special.path)

(herestring_redirect (word) @string)

(file_descriptor) @operator

(expansion "${" @punctuation.special "}" @punctuation.special) @none

(expansion operator: _ @punctuation.special)

(expansion "@" . operator: _ @character.special)

((expansion (subscript index: (word) @character.special))
...
((program . (comment) @keyword.directive @nospell)
